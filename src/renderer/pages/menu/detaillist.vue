<template>
  <div>
    <Form :label-width="90"
      ...
      <FormItem label="品名">
        <Select v-model="search.goods_id"
                style="width:200px;"
      ...
      <FormItem label="备注">
        <Input v-model="search.remark"
               style="width: 200px"
               clearable></Input>
      </FormItem>
      <FormItem label="创建时间">
        <DatePicker :options="datePickerOption"
                    type="daterange"
                    style="width: 200px"
      ...
      <FormItem :label-width="10">
        ...
        <Button style="margin-left:5px;"
                ...
                @click="downloadExcel"
                title="导出"
                :loading="downloadExcelLoading"></Button>
      </FormItem>
    </Form>
    <Table border
           stripe
      ...
      <div>
        <Form ref="formVali"
          ...
          <template v-if="!modalParams.id">
            <FormItem label="品名"
                      prop="goods_id">
              <Select v-model="modalParams.goods_id"
                      style="width:250px;"
                ...
                <Option v-for="(item) in goodsList"
                        :value="item.id"
            ...
            <template v-if="modalParams.goods_id">
              <FormItem label="操作"
                        prop="type"
                        inline>
                <Select v-model="modalParams.type"
                        style="width:50px;"
                        placeholder="">
                  <Option value="+">+</Option>
                  <Option value="-"
                          v-if="modalParams.total_count">-</Option>
                </Select>
                <InputNumber v-model="modalParams.count"
                             :min="0.001"
                             :precision="3"
                             :max="modalParams.type === '-' ? modalParams.total_count : Infinity"></InputNumber>
              </FormItem>
              <FormItem label="实际进价"
                        prop="actual_buy_unit_price"
                        v-show="modalParams.type === '+'">
                <Input v-model.number="modalParams.actual_buy_unit_price"
                       placeholder="非必填，小数位不超过2位的正整数"
                       style="width: 250px"></Input>
              </FormItem>
              <FormItem label="实际售价"
                        prop="actual_sell_unit_price"
                        v-show="modalParams.type === '-'">
          ...
          <FormItem label="备注"
                    prop="remark">
            <Input v-model="modalParams.remark"
                   type="textarea"
                   :rows="4"
